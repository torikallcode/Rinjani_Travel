    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
      <div className=''>
        ...
        <div className='grid grid-cols-4 gap-3 sm:max-w-md'>
          {items.map((item) => {
            return (
              <Link to={"/"} key={item.id} className='aspect-[4/2] flex justify-center items-center p-3 rounded-lg bg-white bg-opacity-5 backdrop-filter backdrop-blur-lg'>
                {item.icon}
              </Link>
            )
          })
